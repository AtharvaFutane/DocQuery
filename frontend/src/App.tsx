...
function AppContent() {
  ...
  const renderChart = (chart: any, index: number) => {
    const colors = chart.colors || ['#6366f1', '#06b6d4', '#f43f5e', '#10b981', '#f59e0b'];
    ...
    <div className="min-h-screen mesh-bg text-primary">
      ...
      <nav className="glass sticky top-0 flex items-center justify-between px-6 py-4 mb-8">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 flex items-center justify-center rounded-xl border border-strong" style={{ background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))' }}>
            <Search className="w-4 h-4" style={{ color: 'var(--accent-text)' }} />
          </div>
          <span className="font-bold text-lg tracking-tight">DocQuery</span>
          <span className="text-[10px] font-mono px-2 py-0.5 rounded-full border border-subtle text-secondary hidden sm:block">
            powered by Groq
          </span>
        </div>
        <div className="flex items-center gap-4">
          ...
          <a href="https://github.com" target="_blank" className="text-secondary hover:text-primary transition-colors">
          ...
          <div className="glass-card p-1 flex items-center gap-1 bg-secondary/50">
            {[
              ...
              <button
                ...
                style={activeTab === tab.id ? { background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))' } : {}}
              >
        ...
        <div className="animate-float-in">
          ...
          {activeTab === 'upload' && (
            <div className="space-y-8">
              <div className="text-center space-y-2 mb-8">
                ...
                <p className="text-secondary opacity-80">Upload financial reports for deep analysis with Groq AI.</p>
              </div>
              ...
              <div className="glass-card p-12 max-w-2xl mx-auto border-dashed border-2 border-subtle hover:border-strong transition-colors cursor-pointer group relative overflow-hidden">
                ...
                <div className="flex flex-col items-center gap-4 relative z-0">
                  <div className="p-4 rounded-full group-hover:scale-110 transition-transform duration-300" style={{ background: 'var(--accent-glow)' }}>
                    <Upload className="w-8 h-8" style={{ color: 'var(--accent-primary)' }} />
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-medium">Drop document here</p>
                    <p className="text-sm text-secondary mt-1">PDF, Word, Excel supported · Max 10MB</p>
                  </div>
                </div>
                {isUploading && (
                  <div className="absolute inset-0 backdrop-blur-sm flex items-center justify-center z-20" style={{ background: 'var(--glass-bg)' }}>
                    <div className="flex flex-col items-center gap-3">
                      <div className="w-8 h-8 border-3 rounded-full animate-spin" style={{ borderColor: 'var(--border-subtle)', borderTopColor: 'var(--accent-primary)' }} />
                      <span className="text-sm font-mono animate-pulse" style={{ color: 'var(--accent-primary)' }}>PROCESSING...</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div>
              {analytics ? (
                <div className="space-y-8">
                  ...
                  <div className="glass-card p-8 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                      <span className="inline-block px-3 py-1 rounded-full text-xs font-mono mb-4 font-semibold" style={{ background: 'var(--accent-glow)', color: 'var(--accent-primary)', border: '1px solid var(--accent-primary)' }}>
                  ...
                  {analytics.table_breakdown && analytics.table_breakdown.total_count > 0 && (
                    <div className="glass-card p-6">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                        <div className="text-center p-3 rounded-lg" style={{ background: 'var(--accent-glow)', border: '1px solid var(--accent-primary)' }}>
                          <div className="text-2xl font-bold" style={{ color: 'var(--accent-primary)' }}>{analytics.table_breakdown.total_count}</div>
                        ...
                        <div className="text-center p-3 rounded-lg bg-cyan-500/10 border border-cyan-500/30">
                          <div className="text-2xl font-bold text-cyan-500">{analytics.table_breakdown.secondary_count}</div>
                        ...
                        <div className="text-center p-3 rounded-lg bg-violet-500/10 border border-violet-500/30">
                          <div className="text-2xl font-bold text-violet-500">{analytics.table_breakdown.analyzed_count}</div>
                        ...
                        {analytics.available_filters.years?.length > 0 && (
                          <div className="flex items-center gap-2">
                            ...
                            <select
                              ...
                              className="px-3 py-1.5 rounded-lg bg-secondary border border-subtle text-sm focus:outline-none focus:border-strong"
                            >
                    ...
                    <div className="grid gap-4">
                      {analytics.key_insights.map((insight, i) => (
                        <div key={i} className="flex gap-4 p-4 rounded-lg bg-secondary/50 border border-transparent hover:border-border-subtle transition-colors">
                          <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-mono text-white" style={{ background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))' }}>
                            {i + 1}
                          </div>
                ...
                <div className="flex flex-col items-center justify-center p-20 glass-card">
                  <div className="p-6 rounded-full mb-6 animate-pulse-glow" style={{ background: 'var(--accent-glow)' }}>
                    <BarChart2 className="w-10 h-10 opacity-50" style={{ color: 'var(--accent-primary)' }} />
                  </div>
          ...
          {activeTab === 'chat' && (
            <div className="glass-card h-[calc(100vh-14rem)] flex flex-col relative overflow-hidden">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full blur-[120px] -z-10 opacity-40" style={{ background: 'radial-gradient(circle, var(--accent-primary), var(--accent-secondary))' }} />

              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                {messages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center opacity-60">
                    <MessageSquare className="w-12 h-12 mb-4 opacity-50" style={{ color: 'var(--accent-primary)' }} />
                    <p className="text-lg font-medium">DocQuery Assistant Ready</p>
                    <p className="text-sm text-secondary mt-1">Powered by Groq AI</p>
                  </div>
                ) : (
                  messages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[85%] p-5 rounded-2xl ${msg.role === 'user'
                        ...
                        style={msg.role === 'user' ? { background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))' } : {}}
                      >
                        <p className="whitespace-pre-wrap leading-relaxed">{msg.content}</p>
                        {msg.citations && msg.citations.length > 0 && (
                          <div className="mt-4 pt-4 border-t border-dashed border-subtle/50">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {msg.citations.map((c, j) => (
                                <button
                                  ...
                                  className="px-2 py-1 text-[10px] rounded-md border border-subtle bg-secondary/50 font-mono flex items-center gap-1 hover:bg-secondary hover:border-strong cursor-pointer transition-colors"
                ...
                {isChatting && (
                  <div className="flex justify-start">
                    <div className="px-4 py-3 rounded-2xl glass flex items-center gap-1.5">
                      <span className="w-2 h-2 rounded-full animate-bounce [animation-delay:-0.3s]" style={{ background: 'var(--accent-primary)' }}></span>
                      <span className="w-2 h-2 rounded-full animate-bounce [animation-delay:-0.15s]" style={{ background: 'var(--accent-secondary)' }}></span>
                      <span className="w-2 h-2 rounded-full animate-bounce" style={{ background: 'var(--accent-primary)' }}></span>
                    </div>
                  </div>
                )}
                <div ref={chatEndRef} />
              ...
              <div className="p-4 glass border-t border-subtle">
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-2 top-2 p-2 rounded-lg text-white hover:opacity-90 disabled:opacity-50 transition-all"
