...
app = FastAPI(
    title="DocQuery API v2.0",
    description="Document Intelligence Platform — powered by Groq",
    version="2.0.0",
    lifespan=lifespan,
)
...
def _sanitize_filename(filename: str) -> str:
    ...
    # Remove any directory separators and keep only the basename
...
async def chat(request: ChatRequest):
    ...
    try:
        response = query_documents(request.query.strip())
...
async def analyze_document_by_id(doc_id: str, force_refresh: bool = False):
    ...
    try:
        ...
        # BUG FIX: Use UPLOAD_FOLDER consistently instead of hardcoded "data/"
...
async def analyze_all_documents():
    ...
    try:
        ...
        for doc in docs:
            # BUG FIX: Use UPLOAD_FOLDER consistently
            file_path = os.path.join(UPLOAD_FOLDER, doc.get('filename'))
            if not os.path.exists(file_path):
                # Try legacy data folder as fallback
                legacy_path = os.path.join(BASE_DIR, "data", doc.get('filename'))
